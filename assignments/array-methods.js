...
let fullNames = [];
runners.forEach(stuff =>
    fullNames.push(`${stuff.first_name} ${stuff.last_name}`))
console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let allCaps = runners.map(capital => capital.first_name.toUpperCase());
console.log(allCaps);
...
let ticketPriceTotal = runners.reduce((accumilator, currentValue) => {
...
// Problem 1 - The event director wants the email of each runner. Use .foreach() to make a new array called Email with the runner's first name and emails.

const Email = [];

runners.forEach(mail => {
  Email.push(`${mail.first_name}: ${mail.email}`)
});
console.log(Email);



// Problem 2 - The small shirts won't be available for the event due to an ordering issue. Get a list of runners with small shirts so they can choose a different size. Return an array named smallShirts that contains information about the runners that have a shirt size of 's' and log the result.

let smallShirts = runners.filter(shirts => shirts.shirt_size === "S");
console.log(smallShirts);




// Problem 3 - The event director needs to have all the runner's last names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each last name into all caps and lof the result.

let allCap = runners.map(capital => capital.last_name.toUpperCase());
console.log(allCap);